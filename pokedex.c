...
void listar_na_pokedex(Pokedex* pokedex){
    ...
    int linha = 0;
    int coluna = 0;
    for (int i = pagina*(3*7); i < MIN(pokedex->qtd_cadastrados, (pagina+1)*21); i++){
        perfil(pokedex->pokemons[i].nome, pokedex->pokemons[i].numero, linha, coluna);
        coluna++;
        if (coluna == 7){
            linha ++;
            coluna = 0;
...
void pesquisar_na_pokedex(int codigo, Pokedex* pokedex){
    for (int i = 0; i < pokedex->qtd_cadastrados; i++){
        if (pokedex->pokemons[i].numero == codigo){
            mostra_pokemon(&pokedex->pokemons[i]);
        }
    }
    // se n achar preciso informar de alguma forma
}
...
void alterar_na_pokedex(int codigo, Pokedex* pokedex){
    for (int i = 0; i < pokedex->qtd_cadastrados; i++){
        if (pokedex->pokemons[i].numero == codigo){
            Pokemon pokemon;
            pokemon.numero = codigo;
            pede_pokemon(&pokemon, 1);
