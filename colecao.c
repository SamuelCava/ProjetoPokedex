...
void listar_colecao(Colecao* colecao, Pokedex* pokedex){
    ...
    int vistos = 0;
    for (int i = 0; i < colecao->capturados; i++){
        //printf("%d\n", colecao->codigo_capturados[i]);
        for (int j = 0; j < pokedex->qtd_cadastrados; j++){
            if(pokedex->pokemons[j].numero == colecao->codigo_capturados[i]){
                if (vistos >= pagina*3*7){
                    perfil(pokedex->pokemons[j].nome, pokedex->pokemons[j].numero, linha, coluna);
                    coluna++;
                    if (coluna == 7){
                        linha ++;
                        coluna = 0;
...
void pesquisar_colecao(int codigo, Colecao* colecao, Pokedex* pokedex){
    for (int i = 0; i < colecao->capturados; i++){
        if (colecao->codigo_capturados[i] == codigo){
            pesquisar_na_pokedex(codigo, pokedex);
        }
    }
}
