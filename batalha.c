...
void batalha_par_impar(Pokemon* pokemon,Mochila* mochila,Pokedex* pokedex, Pokemon pokemons_jogador, Pokemon pokemons_adversario){
    ...
    for (int i = 0; i < pokedex->qtd_cadastrados; i++){
            ...
            if (pokedex->pokemons[i].numero == mochila->codigos_pokemon[j]){
                ...
                pokemons_jogador = pokemon[i];
                pokemons_adversario = pokemon[i];
