...
void batalha_par_impar(Mochila* mochila,Pokedex* pokedex, int qtd[]){
    ...
    if (mochila->pokemons_mochila == 0){
        printf("Voce não tem pokemons\n");
        return;
    }
    printf("Com qual pokemon deseja batalhar? ");
    for (int i = 0; i < mochila->pokemons_mochila; i++){
        for (int j = 0; j < pokedex->qtd_cadastrados; j++){
            if (pokedex->pokemons[j].numero == mochila->codigos_pokemon[i]){
                printf("[%d] %s\n", i+1, pokedex->pokemons[j].nome);
            }
        }
    }
    int escolha;
    ...
    printf("Voce acaba de entrar em uma batalha contra %s\n", pokemons_adversario.nome);
...
// Definindo o valor do adversario através da funcao RAND.
        ...
        switch(menu_batalha){
            ...
            printf("Seu oponente tem %d pontos de vida\n", pokemons_adversario.hp);
        }//while
